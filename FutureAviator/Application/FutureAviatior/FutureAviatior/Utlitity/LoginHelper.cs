...
namespace FutureAviator.Utlitity
{
    public class LoginHelper
    {
        ...
        public bool AuthenticateUser(string EmailID, string Password)
        {
            ...
            Password = HashPassword(Password, DefaultSalt, sha);
            DataAccess.User user = objUser.GetUserDetails(EmailID, Password);
            //DataAccess.User user = objUser.Database.Users.Where(i => i.Email == EmailID && i.Password == Password && i.IsActive == true).FirstOrDefault();
            if (user == null)
            {
                return false;
            }
        ...
        public void AssignSessionData(DataAccess.User user)
        {
            CurrentUser cUser = new CurrentUser();
            int langCode = (int)cUser.LanguageCode;    //save current language//

            cUser.DisplayName = user.DisplayName;
            cUser.Email = user.Email;
            //CurrentUser.Password = "123456";
            cUser.UserID = user.UserID;
            cUser.UserTypeID = user.UserTypeID;
            cUser.LanguageCode = langCode == 1 ? ApplicationLanguages.en : ApplicationLanguages.ar;
            SessionManager.CurrentUser(cUser);
        }
    }
}