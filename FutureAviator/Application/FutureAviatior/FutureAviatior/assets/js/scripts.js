...
$(document).ready(function () {
    $(".nicinstance").each(function (index) {
        new nicEditor({ maxHeight: 100 }).panelInstance($(this).attr('id'));
    });
});
...
function AjaxModalContent(URL) {
    jQuery('#ajax-modal').modal('show', { backdrop: 'static' });
    jQuery.ajax({
        url: URL,
        success: function (response) {
            jQuery('#ajax-modal .modal-body').html(response);
        }
    });
}
function ShowJobResponseDetails(JobResponseID) {
    var URL = "JobResponses.aspx?JobDetails=" + JobResponseID;
    AjaxModalContent(URL);
}
function SetToolTip() {
...
function SetAdminMenu() {
    ...
    var isSet = false;
    $("#AdminMenu a").each(function (a, href) {
        if (!isSet) {
            var linkObject = $(this);
            var link = "" + href;
            ...
            if (documentURL == LinkPageName) {
                if ($(linkObject).parents(".sub-menu").length > 0) {
                    ...
                    $(linkObject).parents("li:first").addClass("open");
                    isSet = true;
                }
            }
        }
